"""Minimal synthetic Stage 1 -> Stage 2 smoke for experiment 032."""
...
ARTIFACT_ROOT = ROOT / "artifacts" / "032" / "smoke"
...
def main():
    ...
    if encoder_config["type"] != "master":
        raise AssertionError("Default config does not enable experiment 032")
    ...
    generator = torch.Generator().manual_seed(32042)
    ...
    # Capture the actual Stage 1 path, including the unchanged RevIN, the
    # gate's adapted canonical input/output shapes, and the preserved
    # spatial-first attention order of experiment 029.
    call_order = []
    gate_shapes = {}
    ...
    try:
        ...
        for handle in handles:
            handle.remove()
    expected_order = [name for name, _module in ordered_modules] + ["RevIN"]
    ...
    stage1_checkpoint = checkpoint_dir / "spatial-first-gated-stage1-smoke.ckpt"
    ...
    stage2_checkpoint = checkpoint_dir / "spatial-first-gated-stage2-smoke.ckpt"
    ...
    report = {
        ...
        "experiment": {
            "id": "032",
            "name": "prism-spatial-first-market-gated-stage1-encoder",
        },
