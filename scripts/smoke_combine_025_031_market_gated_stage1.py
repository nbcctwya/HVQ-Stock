"""Minimal end-to-end Stage 2 smoke for experiment 035.

Experiment 035 = 031 market-gated MASTER-style Stage 1 + 025 Stage 2 (010
Shared-Routed MoE + 019 Quantization Confidence Adapter).  This smoke reuses
the exact Stage 1 provenance recorded by experiment 031
(artifacts/031/run/.stage1.done), verifies the market-gated MASTER-style
encoder structure (RevIN -> Market Gate -> Feature Transform -> Input
Projection -> PositionalEncoding -> TAttention -> SAttention ->
TemporalAttention -> Projection MLP, no GRU), verifies strict Stage 1
loading, zero-init bitwise equivalence with the adapter-off model, exercises
shared+routed forward/backward, adapter optimization, strict Stage 2
checkpoint round-trip, validation/test inference, and the standard prediction
format consumed by the Phase 2/backtest pipeline.  It also records the
quantization-confidence diagnostics required for Phase 2 analysis (q_error
statistics, correction magnitude, adapter norms/gradients, shared/routed
gradient health).  Diagnostics are observational only and do not modify the
training objective.
"""
...
ARTIFACT_ROOT = ROOT / "artifacts" / "035" / "smoke"
STAGE1_MARKER = ROOT / "artifacts" / "031" / "run" / ".stage1.done"
# Final Experiment Commit of experiment 031 (exp/031-prism-market-gated-stage1-encoder).
EXPECTED_031_COMMIT = "40f8740523656e2d9ffa7523412fb02d5d4b33b9"
EXPECTED_CHECKPOINT_BYTES = 13494709
EXPECTED_CHECKPOINT_MD5 = "41158ad69c45a0c62a792342acba6510"
...
def resolve_stage1_checkpoint():
    """Resolve and validate the exact Stage 1 provenance of experiment 031."""
    ...
    if marker.get("commit") != EXPECTED_031_COMMIT:
        raise AssertionError(
            f"031 Stage 1 marker commit {marker.get('commit')} != expected "
            f"031 commit {EXPECTED_031_COMMIT}"
        )
    checkpoint = Path(marker["best"])
    if not checkpoint.is_file():
        # The 031 marker records only the checkpoint filename; resolve it
        # against the marker's own run checkpoints directory.
        checkpoint = STAGE1_MARKER.parent / "checkpoints" / checkpoint.name
    if not checkpoint.is_file() or checkpoint.stat().st_size == 0:
        raise FileNotFoundError(f"031 Stage 1 checkpoint missing or empty: {checkpoint}")
...
def verify_master_encoder_structure(model, feature, market):
    encoder = model.encoder
    master = getattr(encoder, "master_encoder", None)
    if not isinstance(master, MASTERStyleEncoder):
        raise AssertionError("Stage 1 encoder is not MASTER-style (master_encoder missing)")
    if not isinstance(master.tatten, TAttention):
        raise AssertionError("MASTER encoder temporal attention block missing")
    if not isinstance(master.satten, SAttention):
        raise AssertionError("MASTER encoder spatial attention block missing")
    if not isinstance(master.temporalatten, TemporalAttention):
        raise AssertionError("MASTER encoder temporal aggregation block missing")
    if not isinstance(encoder.feature_gate, Gate):
    ...
    modules = [
        ...
        ("Input Projection", master.x2y),
        ("PositionalEncoding", master.pe),
        ("TAttention", master.tatten),
        ("SAttention", master.satten),
        ("TemporalAttention", master.temporalatten),
    ...
    try:
        with torch.no_grad():
            encoder(model.revin(feature, mode="norm"), market)
    finally:
        for handle in handles:
            handle.remove()
    if seen != [name for name, _ in modules]:
        raise AssertionError(f"Stage 1 data flow order changed: {seen}")
    return True


def verify_gate_reads_only_last_market_timestep(model, feature, market):
    encoder = model.encoder
    ...
    with torch.no_grad():
        ...
        base_latent = encoder(feature_normalized, market)
        history_latent = encoder(feature_normalized, changed_history)
        current_latent = encoder(feature_normalized, changed_current)
    if not torch.equal(base_gate, history_gate):
        raise AssertionError("Past market timesteps leaked into Market Gate")
    if not torch.equal(base_latent, history_latent):
        raise AssertionError("Past market timesteps leaked into Stage 1 latent")
    if torch.equal(base_gate, current_gate):
        raise AssertionError("Current market state did not affect Market Gate")
    if torch.equal(base_latent, current_latent):
        raise AssertionError("Current market state did not affect Stage 1 latent")
    return True
...
def main():
    ...
    result_dir = ARTIFACT_ROOT / "res" / "combine_025_031_smoke"
    ...
    if checkpoint_bytes != EXPECTED_CHECKPOINT_BYTES:
        raise AssertionError("031 Stage 1 checkpoint size changed")
    if checkpoint_md5 != EXPECTED_CHECKPOINT_MD5:
        raise AssertionError("031 Stage 1 checkpoint MD5 changed")
    ...
    if predictor_cfg["quantization_confidence_adapter"] is not True:
        raise AssertionError("configs/config.yaml must keep 025 quantization_confidence_adapter")
    encoder_cfg = config["vqvae"]["encoder"]
    if encoder_cfg.get("type") != "master":
        raise AssertionError("Default config must select the 031 market-gated MASTER encoder")
    if encoder_cfg.get("temporal_num_heads") != 2 or encoder_cfg.get("spatial_num_heads") != 2:
        raise AssertionError("031 MASTER encoder heads changed")
    if encoder_cfg.get("temporal_dropout") != 0.1 or encoder_cfg.get("spatial_dropout") != 0.1:
        raise AssertionError("031 MASTER encoder dropout changed")
    if encoder_cfg.get("market_gate") != {
        "input_dim": 63,
    ...
    # adapter is the only Stage 2 code difference inside experiment 035.
    ...
    for name, counts in strict_load.items():
        if counts["missing"] != 0 or counts["unexpected"] != 0:
            raise AssertionError(f"031 Stage 1 strict load failed for {name}: {counts}")
    ...
    feature = train_batch[:, :, GROUP_SLICES["feature"]]
    prior = train_batch[:, -1, GROUP_SLICES["prior"]]
    market = train_batch[:, :, GROUP_SLICES["market"]]

    master_structure_ok = verify_master_encoder_structure(model, feature, market)
    gate_scope_ok = verify_gate_reads_only_last_market_timestep(model, feature, market)
    ...
    with torch.no_grad():
        ...
        base_output = base(feature, prior, market)
        adapted_output = model(feature, prior, market)

    if h_batch.shape[-1] != 128:
        raise AssertionError("031 Stage 1 pre-VQ latent h is not 128-dim")
    ...
    model.eval()
    with torch.no_grad():
        h_after = model.encoder(model.revin(feature, mode="norm"), market)
    ...
    checkpoint_path = checkpoint_dir / "combine-025-031-smoke.ckpt"
    torch.save(
        {
            "epoch": 0,
            "global_step": 1,
    ...
    report = {
        ...
        "stage1": {
            ...
            "expected_031_commit": EXPECTED_031_COMMIT,
            ...
            "encoder_structure": (
                ...
                "TemporalAttention->Projection MLP"
            ),
            "master_structure_verified": master_structure_ok,
            "market_gate_present": True,
            "gate_reads_only_last_market_timestep": gate_scope_ok,
