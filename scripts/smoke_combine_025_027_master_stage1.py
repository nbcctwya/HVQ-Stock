"""Minimal end-to-end Stage 2 smoke for experiment 033.

Experiment 033 = 027 MASTER-style Stage 1 + 025 Stage 2 (010 Shared-Routed
MoE + 019 Quantization Confidence Adapter).  This smoke reuses the exact
Stage 1 provenance recorded by experiment 027 (artifacts/027/run/.stage1.done),
verifies the MASTER-style encoder structure (TAttention -> SAttention ->
TemporalAttention, no GRU, no Market Gate), verifies strict Stage 1 loading,
zero-init bitwise equivalence with the adapter-off model, exercises
...
ARTIFACT_ROOT = ROOT / "artifacts" / "033" / "smoke"
STAGE1_MARKER = ROOT / "artifacts" / "027" / "run" / ".stage1.done"
# Final Experiment Commit of experiment 027 (exp/027-prism-master-stage1-encoder).
EXPECTED_027_COMMIT = "a0962cbe3eac5e21f4d71e23cf18e2e8f298c7dd"
EXPECTED_CHECKPOINT_BYTES = 13370949
EXPECTED_SPLITS = {
...
def resolve_stage1_checkpoint():
    """Resolve and validate the exact Stage 1 provenance of experiment 027."""
    ...
    if marker.get("commit") != EXPECTED_027_COMMIT:
        raise AssertionError(
            f"027 Stage 1 marker commit {marker.get('commit')} != expected "
            f"027 commit {EXPECTED_027_COMMIT}"
        )
    checkpoint = Path(marker["best"])
    if not checkpoint.is_file():
        # The 027 marker records only the checkpoint filename; resolve it
        # against the marker's own run checkpoints directory.
        checkpoint = STAGE1_MARKER.parent / "checkpoints" / checkpoint.name
    if not checkpoint.is_file() or checkpoint.stat().st_size == 0:
        raise FileNotFoundError(f"027 Stage 1 checkpoint missing or empty: {checkpoint}")
...
def verify_master_encoder_structure(model, feature):
    encoder = model.encoder
    ...
    if seen != [name for name, _ in modules]:
        raise AssertionError(f"MASTER encoder data flow order changed: {seen}")
    return True
...
def main():
    ...
    result_dir = ARTIFACT_ROOT / "res" / "combine_025_027_smoke"
    ...
    if checkpoint_bytes != EXPECTED_CHECKPOINT_BYTES:
        raise AssertionError("027 Stage 1 checkpoint size changed")
    ...
    if encoder_cfg.get("type") != "master":
        raise AssertionError("Default config must select the 027 MASTER-style encoder")
    if encoder_cfg.get("temporal_num_heads") != 2 or encoder_cfg.get("spatial_num_heads") != 2:
        raise AssertionError("027 MASTER encoder heads changed")
    if encoder_cfg.get("temporal_dropout") != 0.1 or encoder_cfg.get("spatial_dropout") != 0.1:
        raise AssertionError("027 MASTER encoder dropout changed")
    ...
    # adapter is the only Stage 2 code difference inside experiment 033.
    ...
    for name, counts in strict_load.items():
        if counts["missing"] != 0 or counts["unexpected"] != 0:
            raise AssertionError(f"027 Stage 1 strict load failed for {name}: {counts}")
    ...
    master_structure_ok = verify_master_encoder_structure(model, feature)
    ...
    if h_batch.shape[-1] != 128:
        raise AssertionError("027 Stage 1 pre-VQ latent h is not 128-dim")
    ...
    checkpoint_path = checkpoint_dir / "combine-025-027-smoke.ckpt"
    torch.save(
        {
            "epoch": 0,
            "global_step": 1,
