"""Minimal end-to-end Stage 2 smoke for experiment 034.

Experiment 034 = 028 temporal-attention Stage 1 + 025 Stage 2 (010
Shared-Routed MoE + 019 Quantization Confidence Adapter).  This smoke reuses
the exact Stage 1 provenance recorded by experiment 028
(artifacts/028/run/.stage1.done), verifies the temporal-attention encoder
structure (Input Projection -> PositionalEncoding -> TAttention ->
TemporalAttention -> CrossAssetTransformer; no GRU, no SAttention, no Market
Gate), verifies strict Stage 1 loading, zero-init bitwise equivalence with
the adapter-off model, exercises shared+routed forward/backward, adapter
optimization, strict Stage 2 checkpoint round-trip, validation/test
inference, and the standard prediction format consumed by the Phase
2/backtest pipeline.  It also records the quantization-confidence
diagnostics required for Phase 2 analysis (q_error statistics, correction
magnitude, adapter norms/gradients, shared/routed gradient health).
Diagnostics are observational only and do not modify the training objective.
"""
...
ARTIFACT_ROOT = ROOT / "artifacts" / "034" / "smoke"
...
def main():
    ...
    result_dir = ARTIFACT_ROOT / "res" / "combine_025_028_smoke"
    ...
    # adapter is the only Stage 2 code difference inside experiment 034.
    ...
    checkpoint_path = checkpoint_dir / "combine-025-028-smoke.ckpt"
    torch.save(
        {
            "epoch": 0,
            "global_step": 1,
