"""Minimal synthetic smoke for experiment 023.

Validates the external corrected PRISM-VQ Stage 1 checkpoint, exact initial
equivalence to the baseline prediction forward, detached full continuous
residual, adapter optimization, strict Stage 2 checkpoint round-trip, and
standard prediction output compatibility.
"""
...
ARTIFACT_ROOT = ROOT / "artifacts" / "023" / "smoke"
STAGE1_CHECKPOINT = (
    ROOT
    / "artifacts"
    / "baseline"
...
def build_config(adapter=True):
    ...
    config["predictor"]["residual_correction_adapter"] = adapter
    return config
...
def main():
    ...
    result_dir = ARTIFACT_ROOT / "res" / "residual_correction_smoke"
    ...
    if config["predictor"]["residual_correction_adapter"] is not True:
        raise AssertionError("Default config must enable experiment 023")
    ...
    if not adapter_zero_init or adapter.in_features != 128 or adapter.out_features != 128:
        raise AssertionError("Adapter must be zero-initialized Linear(128, 128)")
    ...
    with torch.no_grad():
        ...
        z_q, _, (_, _, base_vq_idx) = model.quantizer(h_batch)
        z_q = z_q.detach()
        expected_residual = h_batch - z_q
        residual = model.quantization_residual(h_batch, z_q)
        z_stage2 = model.build_stage2_latent(h_batch, z_q)
        base_output = base(feature, prior)
        adapted_output = model(feature, prior)

        # Quantizer assignment / codebook must be untouched by the adapter.
        _, _, (_, _, adapted_vq_idx) = model.quantizer(h_batch)
        codebook_before = model.quantizer.embedding.weight.clone()

        # Non-zero adapter must map different residuals to different corrections.
        model.residual_correction_adapter.weight.copy_(torch.eye(128) * 0.5)
        model.residual_correction_adapter.bias.fill_(0.1)
        other_h = h_batch + torch.randn_like(h_batch)
        delta_a = model.build_stage2_latent(h_batch, z_q) - z_q
        delta_b = model.build_stage2_latent(other_h, z_q) - z_q
        model.residual_correction_adapter.weight.zero_()
        model.residual_correction_adapter.bias.zero_()

    residual_exact = torch.equal(residual, expected_residual)
    residual_detached = not residual.requires_grad
    ...
    assignment_untouched = torch.equal(base_vq_idx, adapted_vq_idx)
    direction_sensitive = not torch.equal(delta_a, delta_b)
    if not residual_exact:
        raise AssertionError("Residual does not match h - z_q")
    if not residual_detached:
        raise AssertionError("Residual must be detached from frozen Stage 1")
    ...
    if not assignment_untouched:
        raise AssertionError("Quantizer assignment changed")
    if not direction_sensitive:
        raise AssertionError("Non-zero adapter is insensitive to residual direction")
    ...
    if adapter_weight_grad_l1 <= 0 or adapter_bias_grad_l1 <= 0:
        raise AssertionError("Residual-correction adapter did not receive gradient")
    ...
    if not adapter_updated:
        raise AssertionError("Residual-correction adapter did not update")
    codebook_untouched = torch.equal(
        codebook_before, model.quantizer.embedding.weight
    )
    if not codebook_untouched:
        raise AssertionError("Codebook changed during a Stage 2 update step")

    checkpoint_path = checkpoint_dir / "residual-correction-smoke.ckpt"
    torch.save(
        {
            "epoch": 0,
            "global_step": 1,
    ...
    report = {
        ...
        "stage1": {
            ...
            "assignment_untouched": assignment_untouched,
            "codebook_untouched_after_update": codebook_untouched,
        },
        "residual_correction_adapter": {
            "module": "Linear(128, 128)",
            "zero_initialized": adapter_zero_init,
            "residual_formula_exact": residual_exact,
            "residual_detached": residual_detached,
