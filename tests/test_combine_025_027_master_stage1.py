"""Tests for experiment 033: 027 MASTER-style Stage 1 + 025 Stage 2.

Experiment 033 keeps experiment 025's Stage 2 (Shared-Routed MoE +
Quantization Confidence Adapter) unchanged and replaces only Stage 1: the
original PRISM-VQ Stage 1 is swapped for experiment 027's frozen MASTER-style
Stage 1 (TAttention -> SAttention -> TemporalAttention, no Market Gate).
"""
...
STAGE1_CHECKPOINT = (
    ...
    / "run"
    / "checkpoints"
    / "infucsi300_h128_VQK512_C128_emb128_dl2p10_s42-epoch=5-val_loss=0.5622.ckpt"
)
...
class DefaultConfigTests(unittest.TestCase):
    """The default configs/config.yaml must fully represent experiment 033."""
    ...
    def test_encoder_selects_master_stage1(self):
...
class EncoderStructureTests(unittest.TestCase):
    """GenerateReturn's Stage 1 must be the 027 MASTER-style encoder."""
    ...
    def test_no_gru_and_no_market_gate(self):
        self.assertFalse(
...
class Stage1CheckpointTests(unittest.TestCase):
    """The 027 official Stage 1 checkpoint must strict-load with zero diffs."""

    def setUp(self):
        if not STAGE1_CHECKPOINT.is_file():
            raise unittest.SkipTest(
                f"027 official Stage 1 checkpoint missing: {STAGE1_CHECKPOINT}"
            )
    ...
    def test_loaded_encoder_is_master_structure(self):
